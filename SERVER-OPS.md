...
> unchecked item in §9 — currently `#6 payment-comgate provider methods`. Latest
> session log is §12.**
...
- [ ] **6 — Plugins.** `payment-comgate` (branch `m1-comgate`, dev in `C:\eshop`).
      Client done (create/status/refund/capture+cancel preauth). Provider: `initiatePayment`,
      `getPaymentStatus`, `retrievePayment` done; **still stubbed** (throw "not implemented"):
      `authorizePayment`, `capturePayment`, `refundPayment`, `cancelPayment`, `deletePayment`,
      `updatePayment`, `getWebhookActionAndData` (webhook = the meaty one). TDD, one at a time.
      Deploy flow (build here → onto server → restart) comes AFTER it's built — no wiring yet.

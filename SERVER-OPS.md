...
> unchecked item in §9. The `#6` provider methods are DONE (all 10, TDD); next is the
> M1 wiring — Task 8 (sandbox integration suite) then Tasks 9–12 (demo register + seed,
> storefront, README/PR). Latest session log is §12.**
...
- [x] **1 — Tailscale.** ✅ Server side done 2026-06-28. Installed v1.98.4, authed to
      ...
      *Remaining (user): (a) install Tailscale on PC/phone + log in same account to connect;
      (b) in admin console (login.tailscale.com), disable key expiry for `reet-server`
      (currently expires 2026-12-25, would drop the server off the tailnet).*
- [~] **6 — Plugins.** `payment-comgate` (branch `m1-comgate` merged to `master`, dev in `C:\eshop`).
      **Provider COMPLETE** — all 10 `AbstractPaymentProvider` methods implemented via TDD
      ...
      Commits: `0392645`, `1247a9e`, `556fee3` (2026-06-29).
      **Remaining for M1** (per `docs/superpowers/plans/2026-06-15-m1-comgate.md`): Task 8 opt-in
      sandbox integration suite, Task 9 register provider in demo backend + CZK region seed,
      Task 10–11 storefront (redirect button + return route), Task 12 README + whole-repo gate + PR.
...
- **Contents per run:** `db_<ts>.sql.gz` (pg_dump of `medusa-mente-backend`) +
  `config_<ts>.tar.gz` (`.env` x2, `medusa-config.ts`, `docker-compose.yml`, `medusa-units/`).
...
- **Next:** M1 wiring — Task 8 (opt-in sandbox suite) → Tasks 9–12 (demo register + CZK seed,
  storefront button + return route, README + whole-repo gate + PR).
